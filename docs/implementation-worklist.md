...
- [x] 4.1 实现打开文件窗口静态 UI。完成时间：2026-06-27 21:45
  ...
  - 追加验证记录：2026-07-24 将已由用户验收的阅读卡片暗色阴影扩展到打开文件窗口和设置窗口的主卡片。新增语义变量 `--window-main-card-shadow`：明亮模式继续解析为原有 `--panel-shadow`，有效暗色模式解析为 `0 0 24px -8px rgb(0 0 0 / 52%), 0 0 8px -2px rgb(0 0 0 / 34%)`；仅 `.open-file-window` 与 `.settings-window-frame` 改为消费该变量，打开文件卡片原有 `1px solid var(--border-soft)`、两张主卡片的宽高、定位、`22px` 圆角、背景和内部控件均未调整。TDD 首轮 `pnpm test:unit` 为 193/194（新增静态回归测试按预期失败），实现后为 194/194；新增生产组件 UI QA 后又发现默认变量同时声明在 `html/body/#root` 会让 `#root` 的默认值遮蔽暗色根覆盖，因此先收紧测试再次得到 193/194，再把默认变量与暗色覆盖统一到 `:root` 层后恢复 194/194。新增固定入口 `pnpm qa:open-file-ui`，明亮模式实测主卡片阴影与 `--panel-shadow` 完全相等，暗色模式实测为上述两层零偏移阴影；锁定 `22px` 圆角、`1px` 边框和 `18px` shell padding。由于保留原有 content-box 边框，打开文件卡片边框外沿仍为四边 `17px`，没有借机调整卡片宽度。扩展 `pnpm qa:settings-ui` 后，设置主卡片明亮阴影继续等于 `--panel-shadow`，暗色实测为上述两层零偏移阴影，四边 inset 均为 `18px`、圆角为 `22px`；原有暗色下拉菜单 `0 28px 64px rgba(0,0,0,.72), 0 10px 26px rgba(0,0,0,.58)` 的独立强阴影仍通过。`pnpm qa:reader-ui` 的 1920 明亮/暗色和 1320×560 高 DPI 场景全部通过，暗色大纲/阅读卡片仍为同一组零偏移阴影。已按原始分辨率人工检查 `output/playwright/open-file-ui-light.png`、`output/playwright/open-file-ui-dark.png`、`%TEMP%\only-md-reader-settings-ui-qa.png` 和 `%TEMP%\only-md-reader-settings-ui-dark-dropdown-qa.png`，确认暗色主卡片四周阴影均匀、明亮模式及内部布局未移动。完整复验 `pnpm test`（194/194 并通过类型检查）、`pnpm lint`、`pnpm format:check`、`pnpm build`、`pnpm qa:open-file-ui`、`pnpm qa:settings-ui`、`pnpm qa:reader-ui`、`git diff --check` 均通过；仅保留已知且不阻塞的 Vite 大于 500 kB chunk 提示。执行 `pnpm tauri build --no-bundle --ci` 成功生成 `E:\only_md_reader\.worktrees\reader-uniform-shadow\src-tauri\target\release\only-md-reader.exe`，未生成 MSI/NSIS；产物大小 `45327360` bytes（`43.23 MiB`），时间 `2026-07-24 10:47:02 +08:00`，SHA-256 为 `203A6ACC8CB5E2A6BE944E602BCD72BB76DB6856AB4FD97917E16D08BDEAE139`。
...
- [x] 6.1 实现阅读窗口静态 UI。完成时间：2026-06-29 17:31
  ...
  - 四周均匀阴影修正记录：2026-07-23 按用户确认的 B 方案，将大纲卡片和阅读卡片原本不同且带方向偏移的阴影统一为共享的居中双层阴影：`0 0 24px -8px`（accent 18%）叠加 `0 0 8px -2px`（accent 10%）。两层阴影的水平、垂直偏移均为 `0`，四周等距扩散；未修改卡片 DOM、`18px` 窗口 inset、`336px` 大纲宽度、`30px` 卡片间距、阅读区 `96px` 水平 padding 与正文默认 `860px` 最大宽度，因此卡片宽度和正文宽度保持原状。静态回归测试先在旧实现上得到预期 RED（仍存在独立大纲阴影且两卡片未共用阴影），最小 CSS 修改后 `pnpm test:unit` 转为 GREEN，完整 `pnpm test` 共 193/193 项通过。固定 `pnpm qa:reader-ui` 在 `1920×1080 @ 1x` 明亮、`1920×1080 @ 1x` 暗色和 `1320×560 @ 2x` 明亮三种场景全部通过：两张卡片的浏览器计算阴影严格相同且包含上述两层零偏移值；1920 场景实测大纲 `336px`、间距 `30px`、阅读卡片左边界 `384px`、阅读卡片宽 `1518px`、正文宽 `860px`，1320 场景对应阅读卡片宽 `918px`、正文宽 `726px`，确认布局尺寸未回归。使用原始分辨率人工核对 `output/playwright/reader-ui-desktop-1920-light.png`、`reader-ui-desktop-1920-dark.png` 和 `reader-ui-min-reader-hidpi.png`，三种场景下两张卡片四周均有连续、均衡的阴影，中间间隙未出现异常加深。`pnpm lint`、`pnpm format:check`、`pnpm build` 和 `pnpm tauri build --no-bundle --ci` 均通过；构建仍只有工作清单 13.8 已记录的 Vite 大 chunk 非阻塞警告，未生成安装包。新测试 EXE 为 `E:\only_md_reader\.worktrees\reader-uniform-shadow\src-tauri\target\release\only-md-reader.exe`，大小 `45,327,360` bytes，LastWriteTime `2026-07-23 21:06:02.120 +08:00`，SHA-256 为 `3D37C0EAE67F857079982B996E98F5D26F0AA574BE492C5C1647F678F9B89EC7`。
  - 暗色阴影清晰度修正记录：2026-07-24 针对暗色主题中 Accent 色阴影与深色卡片叠加形成暖色亮带、导致边界发虚的问题，仅在 `data-theme-effective-mode="dark"` 下把共享卡片阴影覆盖为中性黑色双层零偏移阴影：`0 0 24px -8px rgb(0 0 0 / 52%)` 叠加 `0 0 8px -2px rgb(0 0 0 / 34%)`；明亮主题继续使用既有 Accent 18% / 10% 阴影，未增加边框或 inset shadow，也未修改卡片与正文几何参数。静态回归测试先得到预期 RED（192/193，通过项中仅缺暗色覆盖），最小 CSS 修改后 `pnpm test:unit` 转为 GREEN（193/193）。固定 `pnpm qa:reader-ui` 的 `desktop-1920-light`、`desktop-1920-dark`、`min-reader-hidpi` 三个场景全部通过：两张卡片在同一场景中的浏览器计算阴影严格相同；暗色计算值为 `rgba(0, 0, 0, 0.52) 0px 0px 24px -8px` 与 `rgba(0, 0, 0, 0.34) 0px 0px 8px -2px`，明亮计算值仍为原 Accent 两层阴影。1920 场景继续实测大纲宽 `336px`、间距 `30px`、阅读卡片左边界 `384px`、阅读卡片宽 `1518px`、正文宽 `860px`；1320 高 DPI 场景阅读卡片宽 `918px`、正文宽 `726px`，确认卡片和正文宽度均未改变。使用原始分辨率人工核对三张新鲜截图，暗色卡片间暖色亮带消失、四周边界更清晰且仍均匀，明亮主题视觉保持不变，高 DPI 布局无回归。`pnpm test`（193/193，并通过 TypeScript 类型检查）、`pnpm lint`、`pnpm format:check`、`pnpm build`、`git diff --check` 和 `pnpm tauri build --no-bundle --ci` 均通过；构建仅有工作清单 13.8 已记录的 Vite 大 chunk 非阻塞警告，未生成安装包。新测试 EXE 为 `E:\only_md_reader\.worktrees\reader-uniform-shadow\src-tauri\target\release\only-md-reader.exe`，大小 `45,327,360` bytes，LastWriteTime `2026-07-24 10:06:00.083 +08:00`，SHA-256 为 `F9456CE95C0139F1787C34348A906BF1685C03FE250656080DF800FC63C55566`。
...
- [x] 13.11 将 Windows MSI 升级策略和默认安装目录调整为 0.1.8 方案。完成时间：2026-07-26 22:03
  ...
  - 已实现：应用版本已同步为 `0.1.8`；WiX 使用稳定 `UpgradeCode` 和 `MajorUpgrade`，`RemoveExistingProducts` 调度在 `afterInstallInitialize`；默认目录名改为 `iMDReader`；`bundle.windows.allowDowngrades` 显式设为 `false`，低版本 MSI 不能覆盖已安装的高版本；新增红绿契约测试、只读 MSI 数据库检查入口 `pnpm qa:windows-msi` 和 Windows MSI-only 发布工作流守门。
  - 可见升级提示修正：新增 `OLDER_VERSION_DETECTED` 版本范围和 `UpgradeReadyDlg`。该范围无最低版本，上限为当前 `ProductVersion` 且不包含上限，因此所有严格低于当前版本的同 `UpgradeCode` MSI 都进入“检测到已安装的旧版本”确认页，同版本重装不进入；`WIX_UPGRADE_DETECTED` 继续负责实际移除旧 MSI，`WIX_DOWNGRADE_DETECTED` 继续负责高版本降级拦截。`pnpm qa:windows-msi` 已扩展为读取最终 MSI 的 `Upgrade`、`Dialog`、`Control` 和 `ControlEvent` 表，检查提示文案、按钮、条件路由及版本边界；此处不把数据库检查表述为可见界面或真实升级已验证。
  - MSI 静态验收：`pnpm tauri build --bundles msi`、`pnpm qa:windows-msi` 及指定 0.1.7 MSI 的旧新对比均通过。最新 0.1.8 ProductCode 为 `{DD21E33B-7111-4033-8ACB-69FA2CE3262D}`，0.1.7 ProductCode 为 `{F0632218-E3CA-4AC5-8A47-0C003846DE5B}`，两者共享 UpgradeCode `{D282D977-779F-5080-A3EA-623A41BA26A2}`；0.1.8 的安装目录为 `iMDReader`，`RemoveExistingProducts=1501` 早于 `InstallFiles=4000`。最终 MSI `Upgrade` 表包含旧版升级检测行 `VersionMax=0.1.8 / Attributes=513 / WIX_UPGRADE_DETECTED`，以及高版本检测行 `VersionMin=0.1.8 / VersionMax 为空 / Attributes=2 (OnlyDetect) / WIX_DOWNGRADE_DETECTED`；`LaunchCondition` 表包含 `NOT WIX_DOWNGRADE_DETECTED` 和用户可见错误说明，且未生成当前版本 NSIS。
  - 最终完整回归：在终审修复提交后重新从头执行，`pnpm test`（198/198，含 TypeScript 类型检查）、`pnpm lint`、`pnpm format:check`、`pnpm build`、`cargo test --manifest-path src-tauri\Cargo.toml`（30/30）、`cargo clippy --manifest-path src-tauri\Cargo.toml --all-targets --all-features -- -D warnings`、`pnpm qa:settings-ui`、`pnpm qa:reader-ui`、`pnpm qa:markdown-performance`、`pnpm qa:pdf-export`、`pnpm qa:screenshots` 和 `pnpm tauri build --no-bundle --ci` 均通过。性能样本 1 MiB / 5 MiB / 10 MiB 分别为 546ms / 2603ms / 7226ms。代码主题在明亮 / 暗色模式下分别实测为 `Eva Light Bold` / `Eva Dark Bold`，均有 23 个高亮 token、8 种浏览器计算颜色且 `highlightedTokenMismatches=[]`；固定截图入口生成 6 张截图，未改动受控文件。
  - 终审修复回归：先新增/收紧 `src/app-shell.test.ts` 与 `src/qa-workflows.test.ts`，在旧实现上确认配置、发布标签和 MSI 降级契约三类 RED；最小修复后 `pnpm test`（198/198）、`pnpm lint`、`pnpm format:check`、`cargo test --manifest-path src-tauri\Cargo.toml`（30/30）、`cargo clippy --manifest-path src-tauri\Cargo.toml --all-targets --all-features -- -D warnings`、`pnpm tauri build --bundles msi`、`pnpm qa:windows-msi` 和指定 0.1.7 MSI 对比均通过。发布工作流在构建前读取 `package.json` 与 `src-tauri/tauri.conf.json`，先要求两个版本一致，再要求输入或推送标签严格等于 `v<当前版本>`；`workflow_dispatch` 描述、默认值和错误示例已更新为 `v0.1.8`。
  - 最终产物：测试 EXE 位于 `E:\only_md_reader\.worktrees\windows-msi-upgrade-0.1.8\src-tauri\target\release\only-md-reader.exe`，大小 `45,327,872` bytes，LastWriteTime `2026-07-26 22:33:10.654 +08:00`，SHA-256 `91A4D3262618754102FFD41AE180F684A2E9F99647965294A5144515C7EAA7F4`；MSI 位于 `E:\only_md_reader\.worktrees\windows-msi-upgrade-0.1.8\src-tauri\target\release\bundle\msi\MD极简阅读_0.1.8_x64_zh-CN.msi`，大小 `38,371,328` bytes，LastWriteTime `2026-07-26 22:33:08.824 +08:00`，SHA-256 `54FD6416F0717D330E4697F0561605CA9A78771384056D4D23BB126006245270`。
  - 可见提示修正最终回归：`pnpm test`（200/200，含类型检查）、`pnpm lint`、`pnpm format:check`、`pnpm build`、`cargo test --manifest-path src-tauri\Cargo.toml`（30/30）、`cargo clippy --manifest-path src-tauri\Cargo.toml --all-targets --all-features -- -D warnings`、`pnpm qa:settings-ui`、`pnpm qa:reader-ui`、`pnpm qa:markdown-performance`、`pnpm qa:pdf-export`、`pnpm qa:screenshots`、`pnpm tauri build --no-bundle --ci`、`pnpm tauri build --bundles msi`、`pnpm qa:windows-msi` 和指定 0.1.7 MSI 对比均通过。代码主题在明亮 / 暗色模式下分别实测为 `Eva Light Bold` / `Eva Dark Bold`，均为 23 个 token、8 种浏览器计算颜色且错配为空；1 MiB / 5 MiB / 10 MiB 性能样本分别为 578ms / 2966ms / 9115ms。最终 MSI ProductCode 为 `{741CB815-5658-423E-9CDF-EDEEE3C5EBA6}`；`OLDER_VERSION_DETECTED` 行的 `VersionMin` 为空、`VersionMax=0.1.8`、`Attributes=258`，即包含 `OnlyDetect` 且不包含 `VersionMaxInclusive`；`UpgradeReadyDlg` 及标题、说明、升级/返回/取消控件均存在，条件路由顺序 5 高于普通确认页路由顺序 4，且未生成当前版本 NSIS。
  - 可见提示修正最终产物：测试 EXE 位于 `E:\only_md_reader\.worktrees\windows-msi-upgrade-0.1.8\src-tauri\target\release\only-md-reader.exe`，大小 `45,327,872` bytes，LastWriteTime `2026-07-27 10:37:20.795 +08:00`，SHA-256 `E23BBECE611B0863EFD57BF56EEDF5B629131DEEEB3AB6D1D805D26FDCB0E06C`；MSI 位于 `E:\only_md_reader\.worktrees\windows-msi-upgrade-0.1.8\src-tauri\target\release\bundle\msi\MD极简阅读_0.1.8_x64_zh-CN.msi`，大小 `38,371,328` bytes，LastWriteTime `2026-07-27 10:37:18.616 +08:00`，SHA-256 `B46C8565A4C9B9B44C306CE3CE7B81F36B0CF8E94FF50FB0E9095D522C8D6902`。
  - 未验证：为避免卸载或覆盖宿主现有的 0.1.6 用户安装，本轮未点击“升级”，未执行真实 0.1.6 / 0.1.7 → 0.1.8 升级、同版本重装、高版本降级、0.1.8 全新安装和卸载验收。已实际打开最终 MSI 并确认欢迎页，但安装向导以管理员完整性级别运行，Windows 自动化在尝试进入下一页时返回 `GetCursorPos failed: 拒绝访问。 (0x80070005)`，因此没有把“检测到已安装的旧版本”页面记为可见界面已验证；取消后复核宿主仍为 0.1.6、安装目录仍为 `C:\Program Files\MD极简阅读\`，且 `C:\Program Files\iMDReader` 不存在。本地分支尚未推送，因此 GitHub-hosted Actions 发布流水线尚未实际运行；macOS 实机仍未验证。
...
- [x] 16.12 实现阅读窗口左下角通知栈。完成时间：2026-07-16
  ...
  - 活跃错误最多三条；第四条错误出现时最旧错误先进入关闭动画。错误通知不提供重试按钮，只显示失败原因；所有成功/错误通知右上角提供关闭按钮。
  ...
  - 追加修正记录：2026-07-20 PDF 导出通知改为标题与详情两行结构。成功时第一行显示“PDF文件已导出！”，第二行从原生 `outputPath` 提取 PDF 文件名且不显示路径；失败时第一行显示“PDF导出失败！”，第二行保留资源等待、原生写入或准备阶段产生的具体原因。根据用户实机截图复验，通知框左右两侧与大纲卡片边缘的距离调整为和底部留白相同的 `6px`，最终宽度为 `324px`，相对窗口的左偏移和底偏移均为 `24px`，通知项占满栈宽。所有通知标题、成功文件名和失败原因均固定为单行，超过通知框内部可用宽度时使用省略号截断；视觉截断仅由 CSS 完成，DOM 保留完整原文。回归测试先确认文件名函数、结构化通知类型和两行 DOM/几何断言在旧实现上失败；超长通知 QA 也先确认旧样式返回 `overflow: visible`，再以最小 CSS 修复。修复后 `pnpm test`（191/191）、`pnpm lint`、`pnpm format:check`、`pnpm build`、`pnpm qa:pdf-export`、`pnpm qa:reader-ui` 和 `pnpm tauri build --no-bundle --ci` 全部通过。PDF QA 在 `1440×900 @ 1x` 下实际点击并检查超长成功文件名和超长失败原因，验证两者的 `scrollWidth > clientWidth`、计算样式为 `overflow: hidden`、`text-overflow: ellipsis`、`white-space: nowrap`，并保存 `output/playwright/pdf-export-notification.png`；人工核对成功通知保持单行、省略号可见且原有尺寸与三边间距不变。同一 QA 也验证打印态隐藏。新测试 EXE 为 `E:\only_md_reader\.worktrees\pdf-export-notification\src-tauri\target\release\only-md-reader.exe`，大小 `45,326,848` bytes，SHA-256 为 `84D3E934328D4EF462A6937BF3E60E6A8AC25DEF50B0453C64DAEFF09409E74E`。
  - 关闭按钮正式验证记录：2026-07-20 在正式分支 `codex/pdf-export-notification` 的 `ee0a47c38340a89cbb1d3b2609fae1ee3f2c6db7` 上完成。关闭图标原样使用用户指定的 `M859.00288 178.741248...` 与 `M571.764736 518.862848...` 两条 SVG path；按钮点击区实测为 `24×24px`，图标为 `16×16px`，成功和错误通知均提供右上角关闭按钮。实现仅为成功通知注册 `3,000ms` 自动关闭计时器，错误通知不注册自动关闭计时器并保持常驻；PDF QA 通过 CDP 真实坐标依次点击成功和错误通知的关闭按钮，确认两者进入关闭动画并移除，同时拦截 `setTimeout` / `clearTimeout`，确认提前关闭成功通知时清理的正是该通知创建的 `3,000ms` timer ID。长成功文件名和长错误原因均保持完整 DOM 文本，并实测 `scrollWidth > clientWidth`、`overflow: hidden`、`text-overflow: ellipsis`、`white-space: nowrap`。通知外部几何保持为相对窗口左/下各 `24px`、栈和通知宽度 `324px`、圆角 `14px`，没有因关闭按钮扩张或位移。执行 `pnpm qa:pdf-export` 通过，生成固定模式 `8` 页、`215,925` bytes，全局缩小模式 `5` 页、`209,046` bytes，高 DPI 固定模式 `8` 页、`215,925` bytes；执行 `pnpm qa:reader-ui` 通过，覆盖 `1920×1080 @ 1x` 与 `1320×560 @ 2x`。执行 `pnpm test`（192/192）、`pnpm lint`、`pnpm format:check`、`pnpm build`、`pnpm tauri build --no-bundle --ci` 均通过；构建仅出现工作清单 13.8 已记录的 Vite 大 chunk 非阻塞警告，且 `--no-bundle` 未生成安装包。人工检查浅色 `output/playwright/pdf-export-notification.png`：关闭图标位于通知右上角，标题与详情不重叠，长文件名省略号可见，通知外部几何未回归。暗色通知的固定 QA 与人工截图核对见下一条。新测试 EXE 为 `E:\only_md_reader\.worktrees\pdf-export-notification\src-tauri\target\release\only-md-reader.exe`，大小 `45,327,360` bytes，LastWriteTime `2026-07-20 15:16:45.672 +08:00`，晚于产品代码 HEAD，SHA-256 为 `F9AF9FD0FB4C10527F680AE6402C0465FD1C628A23B3CC9D32A3540DE30E562B`。
  - 暗色通知补充验证记录：2026-07-20 先在固定 `pnpm qa:pdf-export` 中新增 `?pdfExport=error&theme=dark` 场景，并在 fixture 尚未读取 `theme` 参数时确认 RED：`documentElement.dataset.themeEffectiveMode` 实际为 `light`，断言期望 `dark`。最小 GREEN 仅在 `tools/reader-ui-qa.tsx` 首次调用 `applyReaderSettingsToRoot` 前解析 query，只有 `theme=dark` 时把初始 `themeMode` 设为 `dark`，其他场景继续使用 `light`，既有 `pdfExport` 注入行为不变。GREEN 后固定 QA 断言暗色 effective mode、主题背景和非透明边框、通知/按钮/标题/详情计算颜色一致、SVG `fill` 等于继承的 `currentColor`、按钮 `24×24px`、图标 `16×16px`、按钮计算位置为右上 `8px`、标题和详情的可用文字区不进入按钮区域、长错误原因仍为单行 ellipsis；截图前保留通知，再通过 CDP 真实坐标点击关闭并等待 DOM 移除。`pnpm qa:pdf-export` 与 `pnpm qa:reader-ui` 均通过，暗色截图稳定输出为 `output/playwright/pdf-export-notification-dark.png`；使用 `view_image` 人工核对，深色背景上的错误色文字、边框和关闭图标对比清楚，图标位于通知右上角，标题与详情不重叠，省略号可见，外部宽度、圆角和左下定位未回归。复验 `pnpm test`（192/192）、`pnpm lint`、`pnpm format:check`、`pnpm build` 和 `git diff --check` 均通过；构建仍只有工作清单 13.8 的已知大 chunk 警告。本次只修改固定 QA 文件和验证文档，未修改产品代码，因此未重建 EXE；上一条 EXE 仍对应 `ee0a47c` 产品代码之后的正式生产构建，最终验收将再次构建。
  - 暗色通知 QA 硬化记录：2026-07-20 固定 QA 启动时先精确删除既有明亮/暗色通知截图，暗色页面使用 `effectiveMode === "dark"` 与 H1 已渲染的组合条件判定就绪，并等待通知入场动画结束后才测量几何和截图。QA 通过隐藏浏览器探针把真实根变量 `--button-danger-bg` 归一化为计算色，断言错误通知、关闭按钮、标题、详情和 SVG `fill` 均严格等于该颜色，同时静态约束产品 CSS 继续使用 danger token 与 `fill: currentColor`。边框上/右侧严格断言为 `1px solid`；关闭按钮计算位置上/右均为 `8px`，DPR 1 下实际矩形 inset 必须精确等于 `8px + 1px`，即 `9px`，不再接受模糊范围。错误导出场景单独拦截 `setTimeout` / `clearTimeout`，确认出现错误通知前后没有注册任何 `3,000ms` timer ID；真实坐标点击后必须在 `500ms` 内进入 `data-closing`，并在总计 `1,000ms` 内从 DOM 移除。mutation RED 分别临时把暗色边框改为 `0px`，得到预期失败 `0px !== 1px`；另行临时模拟一个 `3,000ms` 定时器，得到预期失败 `1 !== 0`，两处变异随后均已恢复。连续两次 `pnpm qa:pdf-export` 均通过：danger color 均为 `rgb(177, 106, 91)`，上/右实际 inset 均为 `9px`，错误 timer ID 均为空；第一次点击 `23ms` 进入 closing、`343ms` 移除，第二次为 `29ms` 与 `259ms`。随后 `pnpm qa:reader-ui`、`pnpm test`（192/192）、`pnpm lint`、`pnpm format:check`、`pnpm build` 和 `git diff --check` 全部通过，构建仍只有工作清单 13.8 的已知大 chunk 警告。使用 `view_image` 原始分辨率复核新鲜 `output/playwright/pdf-export-notification-dark.png`：关闭图标位于通知右上且留白均衡，标题和详情不与图标重叠，长详情省略号可见，错误文字/边框/图标与暗色背景对比清晰，通知外部宽度、圆角和左下定位未回归。本次仍未修改生产代码，因此不重复构建 EXE。
  - 无边框通知修正记录：2026-07-20 按用户确认的方案直接移除成功和错误通知卡片的边框线；本条取代上两条中关于保留 `1px solid` 边框的旧视觉约束。产品 CSS 使用真实的 `border: 0`，并把卡片 padding 从 `11px 14px` 补偿为 `12px 15px`、关闭按钮定位从上/右 `8px` 补偿为 `9px`，因此卡片总高度、文字位置和关闭按钮相对卡片外沿的实际 `9px` 距离保持不变；错误通知只保留 danger 文字颜色，不再覆盖边框颜色。测试先在旧样式上得到预期 RED：静态断言明确命中旧 `border: 1px` 与 `padding: 11px 14px`，`pnpm qa:pdf-export` 也因无边框 CSS 约束未满足而失败；最小 CSS 修改后转为 GREEN。固定 PDF QA 在亮色和暗色场景均实测计算边框为 `0px none`，关闭按钮计算位置和实际上/右 inset 均为 `9px`，同时继续覆盖 `324px` 宽度、`14px` 圆角、长文本 ellipsis、成功自动关闭、错误常驻、真实坐标关闭与打印态隐藏。人工检查新鲜的 `output/playwright/pdf-export-notification.png` 和 `output/playwright/pdf-export-notification-dark.png`，确认两种主题均无外框线，阴影、圆角、文字截断和关闭按钮布局未回归。正式分支 `codex/pdf-export-notification` 的产品代码提交为 `21bcee8`；`pnpm qa:pdf-export`、`pnpm qa:reader-ui`、`pnpm test`（192/192）、`pnpm lint`、`pnpm format:check`、`pnpm build`、`git diff --check` 和 `pnpm tauri build --no-bundle --ci` 全部通过，构建仅出现工作清单 13.8 已记录的 Vite 大 chunk 非阻塞警告，且未生成安装包。新测试 EXE 为 `E:\only_md_reader\.worktrees\pdf-export-notification\src-tauri\target\release\only-md-reader.exe`，大小 `45,327,360` bytes，LastWriteTime `2026-07-20 16:55:46.131 +08:00`，SHA-256 为 `F86F5F5233DEBE0C8B8F656787AFFCD22350B00B743D2C118ABFFDBD1809FE69`。
