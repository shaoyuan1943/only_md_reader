...
PDF 导出 V1 的目标是让用户从阅读窗口通过显式按钮打开系统打印流程，并在系统界面中选择“另存为 PDF”或等价 PDF 打印机；应用不直接写出指定路径的 PDF 文件。
...
当前 Markdown 渲染 HTML
  ...
  -> Windows：WebView2 原生系统打印对话框
  -> 非 Windows：window.print() 回退
  -> 系统打印预览 / 用户另存为 PDF
```
...
- PDF 始终使用独立的浅色高对比打印主题，不忠实导出当前暗色阅读主题；默认以 A4 和 CSS 页边距排版，最终纸张和打印机选择仍由系统打印窗口决定。
- PDF 仅包含 Markdown 正文，不包含大纲、设置/导出按钮、返回按钮、复制控件、选择浮层、自定义滚动条、卡片阴影、文件路径或文件名。Windows 必须使用 WebView2 `ShowPrintUI(SYSTEM)`，不得使用浏览器打印预览，以避免浏览器自动注入日期、页面标题、`tauri.localhost` 和页码等页眉页脚。
- 导出前必须等待 Markdown 渲染完成、`document.fonts.ready`、正文图片成功或进入既有失败占位状态，以及至少一次最终布局帧；图片持续加载超过约定超时时间时中止本次导出并提示重试，不能悄悄导出缺图文档。
- 图片加载失败不阻断导出，PDF 保留既有失败占位；用户取消系统打印也不显示错误、不改动阅读位置。
- 必须为标题、图片、引用、代码块、表格和块级公式定义打印分页与宽度策略；允许极长单块自然跨页，但不得裁切右侧内容。

V1 明确不做：

- 应用内保存路径选择、静默导出、后台生成或自动打开 PDF。
- 自定义页眉、页脚、页码模板、封面、书签、目录页和 PDF 模板系统。
- Pandoc、LaTeX、外置 Chromium、wkhtmltopdf 或 Rust PDF 绘制库。
- 平台原生 WebView PDF 写文件桥接；该能力作为后续“直接保存 PDF 文件”评估项，不能混入 V1。

验证要求：

- 在现有 `pnpm qa:reader-ui` 基础上新增 `pnpm qa:pdf-export`，该命令必须覆盖打印媒体 CSS 和真实多页 PDF 的基本内容检查。
- Windows 必须实机验证系统打印和 PDF 输出；macOS 未实机验证前必须明确标记为未验证。
